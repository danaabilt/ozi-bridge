...
def _keepalive():
    ...
    class H(BaseHTTPRequestHandler):
        def do_GET(s): s.send_response(200); s.end_headers(); s.wfile.write(b"OZIbridge alive")
        def do_HEAD(s): s.send_response(200); s.end_headers()
        def log_message(s,*a): pass
    try: HTTPServer(("0.0.0.0", port), H).serve_forever()
    except Exception as e: log.warning(f"keepalive: {e}")
...
def main():
    ...
    print("🛰 OZIbridge запущен."); app.run_polling(allowed_updates=Update.ALL_TYPES)
